...
-   **Code Book**: A slide-out panel with tabbed navigation provides documentation across three sections: **Commands** (in-game function reference), **Authoring** (lesson creation guide from `docs/LESSON_AUTHORING_GUIDE.md`), and **Tech** (technical documentation from `docs/TECHNICAL_DOCUMENTATION.md`). Content is lazy-loaded from source files.
